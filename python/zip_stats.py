...
###############################################################################
# Define Function to call getZipCodesStats
###############################################################################
def zip_stats(zipcode, start_date, end_date, api_key):

	# Set Up URL for API Call
	url = trulia + urllib.urlencode({'library':'TruliaStats', 'function':'getZipCodeStats', 'zipCode':zipcode, 'startDate':start_date, 'endDate':end_date, 'apikey':api_key}) # Create url

	# Connect to API & retrive data
	page = urllib.urlopen(url)
	xml_string = page.read()

	# Create dictionary for storage of parsed xml
	output = {"date":[], "zipCode":[], "type":[], "properties":[], "medianPrice":[], "avgPrice":[]}
